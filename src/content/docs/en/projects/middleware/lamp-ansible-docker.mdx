...
This project serves as the baseline for cloud migration:
1.  **Azure Container Registry (ACR):** Move the build process to the cloud instead of building on the host.
2.  **Azure App Service:** Deploy this Web container using the image from ACR.
3.  **Azure Key Vault:** Replace the local `secrets.yml` file with cloud-managed secrets.
