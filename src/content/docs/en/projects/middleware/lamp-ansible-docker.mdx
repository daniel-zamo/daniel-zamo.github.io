...
The deployment logic follows the **SOP-LINUX-ANS-01** standard, separating inventories, playbooks, and source code. This structure enables pre-deployment validations and secure dependency management.

<Steps>

1.  **Inventory Definition**

    Located in the standard development path, we target the Docker host of our infrastructure.
...
2.  **Playbook Structure**
    ...
    The code ensures the Docker network exists, creates persistent volumes, and launches containers.
    
    > **Note:** Since the playbook runs from the `playbooks/` folder, the source code path is adjusted to `../src`.
        ...
        - name: Deploy Web Server (Apache+PHP)
          community.docker.docker_container:
            ...
            networks:
              - name: "{{ project_name }}-net"
            volumes:
              # Mount source code (Move up one level relative to playbook)
              - ../src:/var/www/html
            labels:
              com.role: "frontend"
    ```
...
3.  **Application Code (Proof of Concept)**
    ...
    Located at the project root for easy editing. We inject a simple PHP script that validates internal DNS connectivity.
...
4.  **Execution & Verification**
    ...
    Thanks to the standard `ansible.cfg` configuration, there is no need to manually specify the inventory file.
