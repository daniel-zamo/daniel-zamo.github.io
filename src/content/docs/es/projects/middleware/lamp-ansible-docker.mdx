...
graph TD
    ...
    subgraph Control_Node ["Nodo de Control (Dev)"]
        style Control_Node fill:#eceff1,stroke:#cfd8dc
    ...
    subgraph Remote_Host ["Servidor de Aplicaciones (Ubuntu 24.04)"]
        style Remote_Host fill:#0288d1,stroke:#cfd8dc,stroke-width:2px,color:#ffffff
...
Este proyecto sirve como base para la migración a la nube:
1.  **Azure Container Registry (ACR):** Mover el proceso de *build* a la nube en lugar de construir en el host.
2.  **Azure App Service:** Desplegar este contenedor Web usando la imagen de ACR.
3.  **Azure Key Vault:** Reemplazar el archivo `secrets.yml` local por secretos gestionados en la nube.
